...
while True:
    ...
    if ansID == 5: #选择 6.查看当前猜测
        ...
        print('')
        for i in problAns:
            print('[{}%]{}'.format(round(float(i['element']['proba']) * 100), i['element']['name']))
    ...
    if float(progression) >= 95:
        ...
        if ansIsTrue >= 2:
            ...
            AnsInfo = game.SendChoice(problAns['id'])['parameters']['element_informations']
            print('\n该人物已被使用{}次\n上一次使用是{}\n'.format(AnsInfo['times_selected'], AnsInfo['previous']))
            choice.pause()
            # game.CancelSession()
            break
